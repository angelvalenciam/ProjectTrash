...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <button class="btn btn-warning btn-edit" data-id="{{ $recompensa->id }}" data-titulo="{{ $recompensa->titulo }}"
          data-descripcion="{{ $recompensa->descripcion }}" data-precio="{{ $recompensa->precio }}"
          data-imagen="{{ Storage::url($recompensa->imagen) }}" data-bs-toggle="modal" data-bs-target="#editModal">
          Editar
        </button>
        <form action="{{ route('recompensa.destroy', $recompensa->id) }}" method="POST" style="display:inline;">
          @csrf
